...
foreach($cities as $c) {
	$my_cat = array('cat_name' => $c['title'], 'category_description' => '', 'category_nicename' => sanitize_title($c['title']), 'taxonomy' => 'category');
	$ana = wp_insert_category($my_cat);
	
	foreach($c['districts'] as $d) { 
		$my_cat = array('cat_name' => $d, 'category_description' => '', 'category_nicename' => sanitize_title($c['title']).'-'.sanitize_title($d), 'category_parent' => $ana, 'taxonomy' => 'category');
